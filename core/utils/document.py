...
def extract_metadata(file_data: Dict[str, Any]) -> DocumentMetadata:
    ...
    try:
        ...
        if 'name' not in file_data or 'content' not in file_data:
            raise FileProcessingException("Dados do arquivo incompletos: faltam campos obrigatórios")
        ...
        raise FileProcessingException(f"Erro ao processar metadados: {str(e)}")
